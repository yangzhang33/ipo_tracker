...
def _parse_one_issuer(db, issuer: Issuer, force: bool) -> str:
    ...
    if shares_primary is not None and shares_secondary is not None:
        shares_total = shares_primary + shares_secondary
    elif shares_total is None:
        shares_total = shares_primary if shares_primary is not None else shares_secondary  # best single-source fallback
